--!strict
if getgenv().fortune.luaLoaded then
	return
end
getgenv().fortune.luaLoaded = true
...
repeat
	Luna = loadstring(game:HttpGet("https://raw.githubusercontent.com/dsfgtyuajkvujk/xvxvxvxvxvxvx/refs/heads/main/Luna.lua"))
	...
	if not Luna then
		warn("[fortune.lua]: Failed to load Luna, retrying")
	end
...
local Window = Luna:CreateWindow({
  Name = `fortune.lua | {PlaceName}`,
  Subtitle = `{ScriptVersion or "Dev Mode"}`,
  LogoID = "82795327169782",
  LoadingEnabled = true,
  LoadingTitle = `fortune.lua's {ScriptVersion or "Dev Mode"}`,
  LoadingSubtitle = PlaceName,
  ConfigSettings = {
		RootFolder = "fortune.lua",
	...
	HideIdentity = {
		...
		},
		{
			...
			Info = {
				...
				Callback = function()
					Notify("fortune.lua", "UI Destroyed", "user-round-check", "Lucide")
					getgenv().fortune.luaLoaded = nil
					Luna:Destroy()
				end
			}
		}
...
getgenv().CreateUniversalTabs = function()
	...
	Notify("Welcome to fortune.lua", `Loaded in {math.floor((tick() - StartLoadTime) * 10) / 10}s`, "loader", "Lucide")
end
